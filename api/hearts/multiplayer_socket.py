...
def _on_game_complete(game_id: str, runner: MultiplayerRunner, socketio) -> None:
    ...
    try:
        ...
        for seat_idx, user_id in auth_map.items():
            ...
            newly_unlocked = _compute_newly_unlocked(
                old_snapshot, us, won, player_score
            )
            if newly_unlocked:
        ...
        # Only mark as recorded AFTER the commit succeeds so the HTTP
        # fallback in stats_routes.record_game can still work if the
        # commit failed.
        for uid in committed_user_ids:
            _recorded_games.add(f"{uid}:{game_id}")
        ...
        logger.exception(
            "_on_game_complete: failed to record stats for game=%s — "
            "HTTP fallback will handle recording",
            game_id,
        )
        db.session.rollback()
