...
function decodeJwtPayload(
  ...
  try {
    ...
    return JSON.parse(json) as { sub?: string; username?: string; email?: string; exp?: number };
  } catch {
    return null;
  }
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (username: string, password: string): Promise<{ error?: string }> => {
    ...
    if (res.status === 403 && data.code === "EMAIL_NOT_VERIFIED") {
      return { error: "Check your email to verify your account before signing in." };
    }
    if (!res.ok) {
      return { error: (data.error as string) || "Invalid email or password" };
    }
