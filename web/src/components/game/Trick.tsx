 ...
 */
function getCollectEndpoints(
   target: number,
   scale = 1
): Record<number, Endpoint> {
   const s = scale;
   switch (target) {
      case 0:
         return {
...
export const Trick: React.FC<TrickProps> = ({
   ...
   const cardRefCallbacks = useMemo(
      () =>
         [0, 1, 2, 3].map((i) => (el: HTMLDivElement | null) => {
            setCardRef(i, el);
         }),
      [setCardRef]
   );
         ...
         const keyframes: Keyframe[] =
            ep.midX != null
               ? [
                    ...
                    {
                       opacity: 0,
                       transform: `translate(${ep.x}px, ${ep.y}px)`,
                    },
                 ]
               : [
                    { opacity: 1, transform: "translate(0, 0)" },
         ...
         >
            {TABLE_SLOT_ORDER.map(({ index, className }) => {
               ...
               return (
                  <div
                     key={index}
                     className={`${styles.trickTableSlot} ${className}`}
                  >
                     {slotLabels?.[index] && (
                        <div className={styles.trickTableSlotLabel}>
                           {slotLabels[index]}
                        </div>
                     )}
                     <div
                        className={styles.trickSlotEmpty}
                        aria-hidden="true"
                     />
                     {slot && (
                        <div
