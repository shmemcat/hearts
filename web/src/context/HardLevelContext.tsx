...
export function HardLevelProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (stored && VALID_LEVELS.has(stored)) setHardLevelState(stored as HardLevel);
    if (localStorage.getItem(HARD_LEVEL_CHANGED_KEY) === "1") setHasChanged(true);
  }, []);
  ...
  return (
    <HardLevelContext.Provider value={{ hardLevel, setHardLevel, hasChanged }}>
      {children}
